...
%Setup
\label{sec:perf-energy-arm-single}
\begin{figure}[!th]
\centering
...
\figref{fig:single-perf} shows the detailed results
...
\label{sec:perf-energy-arm-multi}
\begin{figure}[!t]
\centering
...
\figref{fig:multi} shows the detailed results.
...
\subsection{CALOREE on multi-core systems}
In addition to ARM big.LITTLE systems we also repeated the experiments described in
\secref{perf-energy-arm-single} and \secref{perf-energy-arm-multi} on dual-socket Linux 3.2.0 system with a
SuperMICRO X9DRL-iF motherboard and two Intel Xeon E5-2690 processors. We used
same benchmarks as described in \secref{setup-leo}. In \figref{fig:single-sum-clover}
and \figref{fig:multi-sum-clover} we describe the summary of our findings. We find that
other than the race-to-idle, CALOREE has the best performance as only being 4.9\% worse than the optimal.
It has the best energy savings as consuming only 6.9\% extra energy than the optimal, whereas race-to-idle
uses 19.3\% more energy.
In the multi-app scenario, CALOREE has significatly better performance than all the other baselines with only 1.53\% worse performance than the optimal. It also has the highest energy savings with only 7\% worse than the optimal. We feel that CALOREE approach is a very versatile approach and it can used on homogenous and hetergenous systems alike.


\begin{figure}[!t]
\centering
  \input{img/LEO-single-app-summary.tex}
  \caption{Summary data for single-app scenario on multicore processors.}
  \label{fig:single-sum-clover}
\end{figure}

\begin{figure}[!t]
\centering
  \input{img/LEO-multi-app-summary.tex}
  \caption{Summary data for multi-app scenario on multicore processors.}
  \label{fig:multi-sum-clover}
\end{figure}
...
\figref{fig:lavamd-is-hard}(a).  \secref{guarantees} presents an analytical
...
\figref{fig:lavamd-is-hard}(b) shows the results, with time on the x-axis
